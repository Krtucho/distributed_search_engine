...
class VectorModel:
    ...
    def doc_terms_data(self, docs: list[Text]):
        """
        Recibe una lista de documentos de tipo Text
        ...
        for doc in docs:
            ...
            for term in terms_freq:
                if not self.doc_terms.get(term):
                    self.doc_terms[term] = {Text.id : {'freq': terms_freq[term], 'tf': terms_freq[term]/max, 'w': 0}}
                else:
                    self.doc_terms[term][Text.id] = {'freq': terms_freq[term], 'tf': terms_freq[term]/max, 'w': 0}
